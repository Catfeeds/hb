...
class Weixinpay {
    ...
    private $config=array(
        ...
        'NOTIFY_URL'         => 'http://lndttx.net/home/Pay/wxnotify', // 接收支付状态的连接  改成自己的域名
        );

    // 构造函数
    public function __construct($back_url=null){
        // 如果是在thinkphp中 那么需要补全/Application/Common/Conf/config.php中的配置
        // 如果不是在thinkphp框架中使用；那么注释掉下面一行代码；直接补全 private $config 即可
        if($back_url)
            $this->config['NOTIFY_URL']=$back_url;
    }
    ...
    **/
     ...
     */
    public function getParameters($out_trade_no=null,$redirect_uri=null){
        // 获取配置项
        $config=$this->config;
        ...
        if (!isset($_GET['code'])) {
            ...
            if(empty($redirect_uri)){
                $redirect_uri=url('home/pay/wxpay','','',true);
            }
