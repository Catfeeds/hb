...
class Setting extends Base{
	...
	public function setshop(){
		...
		if(request()->isAjax()){
		    ...
		    $res=Db::name('shop_info')->field('shop_logo,content,work_time,shop_jw,server_tel')->where('uid',$uid)->update($data);
		    if($res)
		    	$this->success('操作成功');
		    else
		    	$this->error('操作失败');
		...
		$s_info=Db::name('shop_info')->where('uid',$uid)->field('shop_name,shop_logo,content,work_time,shop_jw,server_tel')->find();
