...
class Good extends Base
{
    ...
    public  function upload(){
        ...
        $upload->savePath = './Uploads/';//图片的的保存位置
        if(!$upload->upload()){
            $error['error']=1;
        ...
        $data=array(
                'url'=>str_replace('./',__ROOT__.'/',$upload->savePath).$info[0]['savename'],
                'error'=>0
        );
     ...
     * 
     */
    public function setStatus($model = 'Good', $script = false)
    {
       $table=db('good');
       $status=input('status');
       $good_id=input('ids/d');
       ...
       if($status=='delete'){ //删除
          ...
          if($res){
            ...
            db('good_img')->where('good_id',$good_id)->delete();
            success('删除成功');
          }else{
            error('删除失败');
          }
