...
class Goodtype extends Base
{
    ...
    {
        $t=db('good_category');
        $list=$t->where(array('pid'=>$pid,'level'=>array('lt',3)))->order('sort_order desc')->select();
        $i=0;
        if(is_array($list)){
            $html = '';
                $i++;
        ...
        if (request()->isPost()) {
            ...
            if($pid==0){
               ...
               $data['pid_path']='0-';
                ...
                $p_info=$table->where('id',$pid)->field('pid_path,level')->find();
                if(!isset($p_info) || empty($p_info)){
                    error('上级不存在');
                }
                $data['level']=$p_info['level']+1;
                $data['pid_path']=$p_info['pid_path'];
            }
            ...
            if ($res===false) {
                ...
                $path_data['pid_path']=array('exp',"CONCAT(`pid_path`,'".$id."-')");
